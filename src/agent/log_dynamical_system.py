...
from agent.utils.spatialmath import log_so3, exp_so3
...
class DynamicalSystem():
    ...
    def get_latent_state(self, x_t=None, space='task'):
        ...
        if space == 'task':
            ...
            # spatial map log to tang vect
            # x_t = [batch, state_dim]
            orientation = x_t[..., 3:]  # extract orientation
    ...
    def transition(self, x_t=None, space='task', **kwargs):
        ...
        if self.order == 1:
            # if i map back to the original space, i get an extra dimension.
            # how should dx_t_d converted ? exp_so3 respect the original val? then come back to the original ?
