...
class Params:
    ...
    results_path: str = 'results/post_laurea/discrete_multi_attractor-Spherical-3D/'
    ...
    n_attractors: int = 2
    latent_dynamic_system_type: str = 'multivariate'
    ...
    stabilization_window_size: int = 3  # stability window size
    stabilization_loss: str = 'contrastive'  # options: contrastive, triplet
    contrastive_margin: float = 3.1159e-8  # contrastive loss margin
    contrastive_norm_margin: float = 0  #
    ...
    workspace_boundaries: np.ndarray = np.array([[-1., 1.0],
                                                 [-1., 1.0],  # loaded as Z
                                                 [-1., 1.0]])  # list to provide boundaries when custom boundaries
    ...
    simulated_trajectory_length: int = 1500  # integration length for evaluation
