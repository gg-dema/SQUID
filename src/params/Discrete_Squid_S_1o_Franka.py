...
class Params:
    """ General parameters """
    dataset_name: str = 'franka_npz'  # selects dataset, options: LASA, LAIR, optitrack, interpolation, joint_space, multi_attractors, kuka
    results_path: str = 'results/robot_exp/s2/Franka/imit-'
    ...
    sigma = 0.50                                         # dev std for the multivariate (R^n and S^n) gaussian
    ...
    learning_rate: float = 0.000009                 # AdamW learning rate
    ...
    boundary_loss_weight: float = 0.01              # weight of the boundary loss
    ...
    max_iterations: int = 35000     # maximum number of training iterations
    ...
    workspace_boundaries: str = np.array([      # list to provide boundaries when workspace_boundaries_type = custom
        [0.0,  0.6],
        [-0.3, 0.3],
        [ 0.0, 0.8],
        [-1.0, 1.0],
    ...
    simulated_trajectory_length: int = 20_000  # integration length for evaluation
    ...
    show_plotly = False
    save_all_models = True               # save any model generated : if False save just the model with the best imitation statistics
