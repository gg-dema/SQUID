...
class Params:
    ...
    results_path: str = 'results/s2/box-simple-stab/'
    ...
    sigma: float = 0.5 #$#0.5  # used only in multivariate_s2 --> dev std on multivariative gauss
    ...
    boundary_loss_weight: float = 0.01 #0.002
    cycle_loss_weight: float = 0.0
    cycle_orientation_loss_weight: float = 0.0
    imitation_window_size: int = 10  # 15 original  # imitation window size
    stabilization_window_size: int = 6  # 2 original  # stability window size
    stabilization_loss: str = 'contrastive'  # options: contrastive, triplet
    contrastive_margin: float = 0.000004  # contrastive loss margin
...
# list to provide boundaries when workspace_boundaries_type = custom
    ...
    simulated_trajectory_length: int = 20000  # integration length for evaluation
