...
class Params:
    """ General parameters """
    dataset_name: str = 'multi_attractors'  # options: LASA, LAIR, optitrack, interpolation, joint_space
    results_path: str = 'results/2nd_order_2D_multi_attractors/'
    multi_motion: bool = False  # true when learning multiple motions together
    selected_primitives_ids: str = '13'  # id number from dataset_keys.py, e.g., '2' or '4,0,6'
    ...
    imitation_window_size: int = 15  #original  # imitation window size
    stabilization_window_size: int = 10  # 2 original  # stability window size
    stabilization_loss: str = 'contrastive'  # options: contrastive, triplet
    contrastive_margin: float = 3.33e-7  # contrastive loss margin
