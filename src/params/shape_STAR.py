...
class Params:
    ...
    #results_path: str = 'results/shape/star-contrastive_norm_search/only-orient_with-hard-negative/'
    results_path: str = 'results/post_laurea/shape/star/'
    ...
    spherical_latent_space: bool = False
    ...
    """ Multi Attractors """
    n_attractors: int = 1  # not sure on why, before was 3 attractors. In any case, the imit is not used
    latent_dynamic_system_type: str = "limit cycle"  # option: standard, gaussian, well_known_gaussian, well_known_multivariate, multivariate
    ...
    batch_size: int = 128  # sampling batch size
    batch_size_contrastive_norm: int = 512  # 512
    batch_size_orientation_loss: int = 512  # 512
    learning_rate = 0.00008
    weight_decay = 0.000001
    #learning_rate: float = 0.00009  # used for star --> 0.00049  # AdamW learning rate
    #weight_decay: float = 0.0000001  # AdamW weight decay

    """ Contrastive Imitation """
    imitation_loss_weight: float = 0  # imitation loss weight
    stabilization_loss_weight: float = 5.5
    goal_mapping_loss_weight: float = 0
    shaped_attractors_form: str = "star"  # if none, no shape is enforce to the attractors.
                                          # option: star
                                          #circle_interpolate,
                                          # circle  --> by file
    ...
    cycle_orientation_loss_weight: float = 1.0

    boundary_loss_weight: float = 0.0  
    goal_mapping_decreasing_weight: float = 0.99
    imitation_window_size: int = 2  # 15 original  # imitation window size
    stabilization_window_size: int = 6  # 8 original  # stability window size
    stabilization_loss: str = 'contrastive'  # options: contrastive, triplet
    contrastive_margin: float = 0.0333  # contrastive loss margin
    contrastive_norm_margin: float = 1.0
    print_demos: bool = False  # if true, print the demonstrations and the trajectory generated from them
