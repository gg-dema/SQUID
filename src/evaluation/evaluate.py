...
class Evaluate():
    ...
    def __init__(self, learner, data, params, verbose=True):
        ...
        self.mask2index = data['trajectory 2 attractor index']
    ...
    def get_initial_states_grid(self):
        ...
        self.joint_space = True
        if self.dim_workspace >= 5 and not self.joint_space:

            points_sphere = np.random.uniform(
                low=-1, high=1, size=(self.density ** (self.dim_workspace), self.dim_workspace)
            )
    ...
    ''' 16-april 2025: i suppose that this is not used any more. 
        If i ever re-found this code still under comment, delate.
        
    def stability_metrics_from_pseudo_init(self, primitive_id):

        n_sample = 10

        init_state = self.get_initial_states_demos(primitive_id)
        traj = [init_state[self.mask2index == i] for i in range(self.n_attractors)]
        goals = denormalize_state(self.goals, self.x_min, self.x_max)
        primitive_type = torch.FloatTensor(primitive_id)

        sample = torch.empty(n_sample*self.n_attractors, self.dim_workspace)
        for id in range(self.n_attractors):
            avg = torch.mean(traj[id], axis=0)
            lower = avg - 0.05
            upper = avg + 0.05
            distrib = torch.distributions.Uniform(lower, upper)
            sampled_p = distrib.sample((n_sample, self.dim_workspace))
            sample[id*n_sample:(id+1)*n_sample] = sampled_p

        sample = sample.cuda()
        dynamic_system = self.learner.init_dynamical_system(sample, primitive_type)
        visited_states, _ = dynamic_system.simulate(self.simulated_trajectory_length, space="task")
        attractors = denormalize_state(visited_states[-1, :, :], self.x_min, self.x_max)
        attractors = attractors.reshape(-1, self.n_attractors, self.dim_workspace)
        distances = np.array([np.linalg.norm(attractors[:, i, :] - goals[0, i, :], axis=1) for i in range(self.n_attractors)])
        n_spurious = np.count_nonzero( ~(distances < self.fixed_point_iteration_thr) )

        return n_spurious / (n_sample * self.n_attractors)
    '''
    ...
    def get_stability_metrics(self, attractor, goal):
        ...
        for i in range(self.n_attractors):
            distance_per_goal = np.linalg.norm(attractor - goal[i], axis=1)
            distance_to_goals.append(distance_per_goal)

        distance = np.stack(distance_to_goals).min(axis=0)
        # Find spurious/unsuc. trajectory and mean distance to goal
        n_spurious = np.sum(distance > self.fixed_point_iteration_thr)
        mean_dist_to_goal = np.mean(distance)

        # compute spurious percent for perturbed trajectory:
        # perturbed_spurious_rate = self.stability_metrics_from_pseudo_init(primitive_id=0)
        perturbed_spurious_rate = -10

        # Append results to history
        self.mean_distance_to_goal.append(mean_dist_to_goal)
        self.n_spurious.append(n_spurious)
        self.perturbed_spurious.append(perturbed_spurious_rate)

        # Get results
        metrics_stab = {'n spurious': n_spurious,
                        'mean dist to goal': mean_dist_to_goal,
                        # 'perturbed init spurious rate': 0,
                        'perturbed init spurious rate': perturbed_spurious_rate,
                        }
    ...
    def save_progress(self, save_path, i, model, writer):
        ...
        # Log metrics to tensorboard
            ...
            writer.add_scalar('eval/perturbed_spurious', self.perturbed_spurious[-1], i)
